// HomePage — hero + 3 rules + 4 day cards with progress.
const { useEffect, useState: useStateHome } = React;

function readDayProgress(n) {
  try {
...
function HomePage({ go }) {
  ...
  // Re-read storage when window gets focus / route changes.
  useEffect(() => {
  ...
  // Find the "current" day = first day not fully complete.
  let currentN = 1;
      ...
      <header className="hero">
        ...
        <div className="hero__mascot-row">
          <Button variant="primary" iconEnd="arrowLeft" onClick={() => go({ view: 'day', n: currentN })}>
            ابدأ من اليوم {['١','٢','٣','٤'][currentN-1] || '١'}
          </Button>
          <div className="tv">
              ...
              <Card key={d.n} state={state} onClick={() => go({ view: 'day', n: d.n })}>
                <div className="day-card">
                  <div className="day-card__top">
                    <div className="day-card__n">اليوم {['١','٢','٣','٤'][d.n-1]}</div>
...
window.readDayProgress = readDayProgress;
